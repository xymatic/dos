...
dos.$exception(dos, "AssertException", {
    extends: dos.Exception
});
...
dos.$static(dos, "Util", {
    static: function(This, Super, Public, Protected, Private) {
        ...
        Public.$method("hashString", function(string) {
            var hash = 5381,
                c;
        ...
        Public.$method("assert", function(expression, message) {
            if(!expression) {
                throw new dos.AssertException({
                    what: message || "assertion failed"
                });
            }
        });

        /**
         * helper that ensures the given callback function is invoked once only!
         */
        Public.$method("once", function(cb) {
            var cbCalled = false;
            return function() {
                if(!cbCalled) {
                    cbCalled = true;
        ...
        // TODO: remove when real enum class introduced!
        ...
        /** get containing dir from file path or url.
         *
         * e.g. getBasePath("http://example.org/some/folder/index.html")
         *      will return "http://example.org/some/folder/"
         */
        Public.$method("getBasePath", function(path) {
            return path.split(/[^\/]*$/)[0];
        });

        Public.$method("resizeArray", function(arr, newSize) {
            var res = new Array(newSize);
            var len = Math.min(newSize, arr.length);
            for(var i = 0; i < len; ++i) {
                res[i] = arr[i];
            }
            return res;
        ...
        Public.$method("removeValueFromArray", function(arr, val) {
            var idx;
        ...
        Public.$method("pushUniqe", function(arr, val) {
        ...
        Public.$method("copyProp", function(dst, src, name) {
            return Object.defineProperty(dst, name, Object.getOwnPropertyDescriptor(src, name));
        });

        Public.$method("mergeArrayWithPredicate", function(dst, src, pred) {
            for(var i = 0, srcLen = src.length; i < srcLen; ++i) {
                var elem = src[i];
                for(var j = 0, dstLen = dst.length; j < dstLen; ++j) {
                    if(pred(dst[j], elem)) {
                        dst.push(src[i]);
                        break;
                    }
        ...
        Public.$method("mergeArray", function(dst, src) {
            for(var i = 0, srcLen = src.length; i < srcLen; ++i) {
        ...
        Public.$method("merge", function(dst, src) {
            for(var attrName in src) {
                Public.copyProp(dst, src, attrName);
            }
            return dst;
        });

        Public.$method("mergeRecursive", function(dst, src) {
            for(var attrName in src) {
                if(Array.isArray(src[attrName])) {
                    dst[attrName] = src[attrName].slice(0);
                } else if(typeof src[attrName] === "object") {
                    dst[attrName] = Public.mergeRecursive(dst[attrName] || {}, src[attrName]);
                } else {
                    Public.copyProp(dst, src, attrName);
                }
            }
        ...
        Public.$method("clone", function(o) {
            return Public.mergeRecursive({}, o);
        });

        Public.$method("deepEqual", (function() {
            var arrEqual = function(a, b) {
                if(a.length !== b.length) {
                    return false;
                }
                for(var i = 0, len = a.length; i < len; ++i) {
                    if(!Public.deepEqual(a[i], b[i])) {
                        return false;
                    }
                }
                return true;
            ...
            var objEqual = function(a, b) {
                ...
                // first, check if keys are equal - cheaper than value test on every key
                var i, len;
                ...
                for(i = 0, len = aKeys.length; i < len; ++i) {
                    if(!Public.deepEqual(a[aKeys[i]], b[aKeys[i]])) {
                        return false;
                    }
                }
            ...
            return function(obj1, obj2) {
                ...
                if(Array.isArray(obj1) && Array.isArray(obj2)) {
                    return arrEqual(obj1, obj2);
                }
                if(typeof obj1 === "object" && typeof obj2 === "object") {
                    return objEqual(obj1, obj2);
                }
                return false; // TODO: allow normal equality? return obj1 == obj2;
            };
        })());

        /**
         * for performance comparisions, see [jsperf guid-makers](http://jsperf.com/guid-makers/6)
         */
        Public.$method("getGUID", (function() {
            var ZERO_PADDING = "00000000";
            var GROUPS = [0, 0, 0, 0, 0, 0];
            return function newGUID() {
                GROUPS[0] = Math.floor(Math.random() * 0x10000000).toString(16);
                GROUPS[1] = Math.floor(Math.random() * 0x10000).toString(16);
                GROUPS[2] = (Math.floor(Math.random() * 0x1000) + 0x4000).toString(16);
                GROUPS[3] = (Math.floor(Math.random() * 0x4000) + 0x8000).toString(16);
                GROUPS[4] = Math.floor(Math.random() * 0x1000000).toString(16);
                GROUPS[5] = Math.floor(Math.random() * 0x1000000).toString(16);
                return GROUPS[0] + ZERO_PADDING.slice(0, 8 - GROUPS[0].length) + "-" +
                    GROUPS[1] + ZERO_PADDING.slice(0, 4 - GROUPS[1].length) + "-" +
                    GROUPS[2] + ZERO_PADDING.slice(0, 4 - GROUPS[2].length) + "-" +
                    GROUPS[3] + ZERO_PADDING.slice(0, 4 - GROUPS[3].length) + "-" +
                    GROUPS[4] + ZERO_PADDING.slice(0, 6 - GROUPS[4].length) +
                    GROUPS[5] + ZERO_PADDING.slice(0, 6 - GROUPS[5].length);
            };
        })());

        Private.$const("GuidRegExp", {
            value: /^[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}$/i
        });

        Public.$method("isValidGUID", function(guid) {
            return Private.GuidRegExp.test(guid);
        });

        // copied from http://snipplr.com/view/6889/
        ...
        //  11   == query
        //  12   == fragment
        Private.$const("UriRegExp", {
            value: /^([a-z][a-z0-9+.-]*):(?:\/\/((?:(?=((?:[a-z0-9-._~!$&'()*+,;=:]|%[0-9A-F]{2})*))(\3)@)?(?=(\[[0-9A-F:.]{2,}\]|(?:[a-z0-9-._~!$&'()*+,;=]|%[0-9A-F]{2})*))\5(?::(?=(\d*))\6)?)(\/(?=((?:[a-z0-9-._~!$&'()*+,;=:@\/]|%[0-9A-F]{2})*))\8)?|(\/?(?!\/)(?=((?:[a-z0-9-._~!$&'()*+,;=:@\/]|%[0-9A-F]{2})*))\10)?)(?:\?(?=((?:[a-z0-9-._~!$&'()*+,;=:@\/?]|%[0-9A-F]{2})*))\11)?(?:#(?=((?:[a-z0-9-._~!$&'()*+,;=:@\/?]|%[0-9A-F]{2})*))\12)?$/i
        });

        Public.$method("isValidUri", function(uri) {
            return Private.UriRegExp.test(uri);
        });

        Public.$method("parseUri", function(uri) {
            var x = Private.UriRegExp.exec(uri);
            if(x) {
                return {
                    uri: uri,
                    schema: x[1],
        ...
        /**
         * try to find setImmediate. if not available, fall back to setTimeout(..., 0)
         */
        Public.$method("setImmediate", (function() {
        ...
        /**
         * see {@link setImmediate}
         */
        Public.$method("clearImmediate", (function() {
            if(typeof global !== "undefined" && typeof global.clearImmediate === "function") {
                return function(id) {
                    clearImmediate(id);
                };
            } else {
        ...
        /**
         * for performance comparisions, see [jsperf for-vs-foreach](http://jsperf.com/for-vs-foreach/110)
         */
        Public.$method("forEach", function(arr, cb) {
            for(var i = 0, len = arr.length; i < len; ++i) {
                cb(arr[i]);
            }
        });
        ...
        /**
         * @param {Object} P
         * @param {Function} P.fn function(pN, cb)
         * @param {Array} P.params [p0, p1, ...]
         * @param {Object} P.scope obj
         * @param {Function} P.callback function(err, chunks)
         * @param {Boolean} P.skipNullResults
         */
        Public.$method("forEachAsync", function(P) {
            var Params = new dos.Params(Public.merge({
                skipNullResults: true,
                scope: {}
            }, P));

            var skipNullResults = Params.getType("skipNullResults", dos.Types.Boolean);
            var forEachFn = Params.getType("fn", dos.Types.Function);
            var params = Params.getType("params", dos.Types.Array).slice(0);
            var scope = Params.getType("scope", dos.Types.Object);
            var onDone = Params.getType("callback", dos.Types.Function);

            var results = [];
            var unwindParams = function(callback) {
                if(!params || params.length < 1) {
                    return callback();
                }
                var param = params[0];
                params = params.slice(1);
                forEachFn.call(scope, param, function(err, res) {
                    if(err) {
                        return callback(err);
                    }
                    if(!skipNullResults || (res !== undefined && res !== null)) {
                        results.push(res);
                    }
                    dos.Util.setImmediate(unwindParams.bind(this, callback));
                });
            };
            unwindParams(function(err) {
                onDone(err, results);
            });
        });

        Public.$method("asyncPoll", function(P) {
            P = dos.Util.merge({
                interval: 10
            }, P);
        ...
        Public.$method("makePartial", function(fn) {
            return function() {
